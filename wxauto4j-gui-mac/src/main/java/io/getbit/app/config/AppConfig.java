...
public class AppConfig {
    ...
    /** 解密后数据库输出目录 */
    private String wxDecryptedDbDir = detectDefaultDecryptedDbDir();

    /** 检测操作系统默认微信数据目录 */
    private static String detectDefaultWxDataDir() {
    ...
    /** 检测操作系统默认解密数据库输出目录 */
    private static String detectDefaultDecryptedDbDir() {
        String os = System.getProperty("os.name", "").toLowerCase();
        if (os.contains("win")) {
            // Windows: %TEMP%\wx_decrypted_java
            return System.getenv("TEMP") + "\\wx_decrypted_java";
        } else {
            // macOS: /tmp/wx_decrypted_java
            return "/tmp/wx_decrypted_java";
        }
    }
